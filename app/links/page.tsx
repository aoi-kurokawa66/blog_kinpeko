const SNS_LINKS = [
  {
    title: "YouTube",
    url: "https://www.youtube.com/@aquarium-abyss",
    description: "キンペコの動画コンテンツを配信しています",
    handle: "@aquarium-abyss",
    color: "from-red-600/20 to-red-800/10",
    border: "border-red-500/30",
    accent: "text-red-400",
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
        <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z" />
      </svg>
    ),
  },
  {
    title: "X (Twitter)",
    url: "https://x.com/aquarium621",
    description: "最新情報や日常の様子をツイートしています",
    handle: "@aquarium621",
    color: "from-slate-600/20 to-slate-800/10",
    border: "border-slate-500/30",
    accent: "text-slate-300",
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
        <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
      </svg>
    ),
  },
  {
    title: "Instagram",
    url: "https://www.instagram.com/kinpeco_aquarium",
    description: "キンペコの写真や動画を投稿しています",
    handle: "@kinpeco_aquarium",
    color: "from-pink-600/20 to-purple-800/10",
    border: "border-pink-500/30",
    accent: "text-pink-400",
    icon: (
      <svg viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z" />
      </svg>
    ),
  },
];

export default function LinksPage() {
  return (
    <div className="min-h-screen">
      {/* ページヘッダー */}
      <section className="bg-ocean-800 py-10 md:py-14 border-b border-ocean-500/50">
        <div className="container mx-auto px-5 max-w-5xl">
          <p className="text-cyan-400 text-[0.65rem] font-bold tracking-[0.2em] uppercase mb-2">Links</p>
          <h1 className="text-2xl md:text-3xl font-bold text-ink-primary">リンク集</h1>
          <p className="text-sm text-ink-muted mt-1">SNSや委託販売ショップへのリンク</p>
        </div>
      </section>

      <section className="bg-ocean-900 py-8 md:py-12">
        <div className="container mx-auto px-5 max-w-5xl">

          {/* SNS */}
          <p className="text-[0.65rem] font-bold text-ink-muted tracking-[0.2em] uppercase mb-4">SNS</p>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
            {SNS_LINKS.map((link) => (
              <a
                key={link.title}
                href={link.url}
                target="_blank"
                rel="noopener noreferrer"
                className={`group glass-card-hover overflow-hidden rounded-2xl bg-gradient-to-br ${link.color} border ${link.border} p-5 flex flex-col gap-4`}
              >
                <div className="flex items-start justify-between">
                  <span className={link.accent}>{link.icon}</span>
                  <svg className="w-3.5 h-3.5 text-ink-muted group-hover:text-cyan-400 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
                  </svg>
                </div>
                <div>
                  <p className="text-sm font-bold text-ink-primary group-hover:text-cyan-400 transition-colors mb-0.5">{link.title}</p>
                  <p className="text-[0.65rem] text-ink-muted mb-2">{link.handle}</p>
                  <p className="text-xs text-ink-muted leading-relaxed">{link.description}</p>
                </div>
              </a>
            ))}
          </div>

          {/* 委託販売 */}
          <div className="border-t border-ocean-500/50 pt-8">
            <p className="text-[0.65rem] font-bold text-ink-muted tracking-[0.2em] uppercase mb-4">Shop</p>
            <h2 className="text-base font-bold text-ink-primary mb-3">委託販売ショップ</h2>
            <div className="glass-card p-5">
              <p className="text-sm text-ink-muted">委託販売ショップの情報は準備中です</p>
            </div>
          </div>
        </div>
      </section>
    </div>
  );
}
